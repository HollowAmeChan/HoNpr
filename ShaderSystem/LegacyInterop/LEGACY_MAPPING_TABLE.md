...
| `FORWARD_OUTLINE` / `LIL_OUTLINE` / `lil_vert_outline.hlsl` | lilToon outline pass | `MaterialBlock.LilToonSourceOutlinePass` + `ForwardOutlineLilToonSource` | SimplifyFirst | Derived object-space normal extrusion, width mask, vertex color width mode, and z bias; no lilToon include or old property ABI. |
