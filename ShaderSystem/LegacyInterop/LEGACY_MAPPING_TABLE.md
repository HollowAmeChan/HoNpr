...
| `FORWARD_OUTLINE` / `LIL_OUTLINE` / `lil_vert_outline.hlsl` / `OVERRIDE_OUTLINE_COLOR` | lilToon outline pass | `MaterialBlock.OutlineLilToon` + `ForwardOutlineLilToon` | SimplifyFirst | Component-first outline block using the lilToon algorithm variant: object-space extrusion, width mask, vector map, vertex color direction/width mode, outline texture color, simplified outline lighting, and z bias; no lilToon include or old property ABI. |
