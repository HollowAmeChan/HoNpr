...
preset MaterialPreset.Character_LilToon_Standard
{
    ...
    blocks MaterialBlock.BaseColorTexture MaterialBlock.LilToonDiffuseRamp ...;
    passes UniversalForward ForwardLilToonOutline HoUrpAovOutput DepthOnly ShadowCaster;
    generator CharacterLilToonTemplate;
    shader "Shaders/Generated/LilToon/Standard.shader";
}
```
...
```text
Shaders/ShaderLibrary/Assemblies/CharacterLilToon/
  HoNprCharacterLilToonCommon.hlsl
  HoNprCharacterLilToonLite.hlsl
  HoNprCharacterLilToonStandard.hlsl
  HoNprCharacterLilToonRich.hlsl
  HoNprCharacterLilToonTransparent.hlsl
```
...
允许的 C# 分支是“选择组装器类型”，例如 `CharacterLilToonTemplate`；不允许在 C# 中写 `if Lite then remove specular` 这类结构事实。
...
// Assembly: HoNpr.CharacterLilToon.Standard
// Blocks: MaterialBlock.BaseColorTexture, MaterialBlock.LilToonDiffuseRamp, ...
