...
| 目录类型 | 必须文件 | 人类用途 | AI/工具用途 |
| --- | --- | --- | --- |
| 模板目录 | `TEMPLATE_TABLE.md` | 看 pass 骨架、输入输出、生成目标 | 约束生成器允许使用的模板 |
| Feature Block 目录 | `FEATURE_BLOCK_TABLE.md` | 看每个 block 的 Domain、输入、输出、变体策略 | 校验 block 是否越权消费/生产 |
| Preset 目录 | `PRESET_TABLE.md` | 看每个 preset 启用哪些 block 和 pass | 生成 shader 和校验组合合法性 |
| Generated 目录 | `GENERATED_TABLE.md` | 看生成产物来源和是否可手改 | 校验产物是否与 preset 同步 |
...
| `Templates/` | shader pass 骨架，不写复杂业务 | `TEMPLATE_TABLE.md` |
| `FeatureBlocks/` | 材质功能块 HLSL / 描述 | `FEATURE_BLOCK_TABLE.md` |
| `Presets/` | 静态组合定义 | `PRESET_TABLE.md` |
| `Generator/` | 生成器代码和规则 | `GENERATOR_RULES.md` |
| `GeneratedManifests/` | 每个生成产物的来源 manifest | `GENERATED_TABLE.md` |
...
Templates/
  ...
  Character/
    ...
    CharacterAov.template
    CharacterDepth.template
    CharacterShadow.template
    CharacterOit.template
  Environment/
    EnvironmentForward.template
    EnvironmentAov.template
  Utility/
    DebugLit.template
```
...
Presets/
  PRESET_TABLE.md
  Character/
    Character_Toon_Core.preset.json
    Character_Skin_SSS.preset.json
    Hair_Toon.preset.json
  Environment/
    Environment_PBR.preset.json
  Transparent/
    Transparent_OIT.preset.json
  Debug/
    Character_DebugLit_SSS_OITReady.preset.json
```
...
- 生成前读取 `PRESET_TABLE.md` / `*.preset.json`。
- 校验每个 block 在 `FEATURE_BLOCK_TABLE.md` 中存在。
- 校验每个 block 的 `VariantPolicy` 允许当前 preset 使用。
- 校验 pass 由模板决定，不能由 UI 参数决定。
- 生成 `.shader` 时写入 source mapping 注释。
- 生成产物必须可读、可 diff、可删除重生。
- 强制刷新按钮必须重跑生成器、刷新 `Shaders/Generated/`、重建 `GeneratedManifests/`、执行声明表校验，并通过 `AssetDatabase.ImportAsset(..., ForceUpdate | ForceSynchronousImport)` 导入生成结果。
- 强制刷新不能读取材质 Inspector 当前状态来决定结构；它只能读取 template / block / preset / manifest。
...
- 不手改生成产物主体。
- 必须有 source mapping。
- 每个生成 shader 在 `GENERATED_TABLE.md` 有记录。
- 生成器重跑后 diff 应该稳定。
...
| 测试 | 目的 |
| --- | --- |
| `FeatureBlockTableTests` | 表格里的 block 都有文件或 manifest |
| `PresetTableTests` | preset 引用的 block 都存在 |
| `GeneratedShaderManifestTests` | generated shader 来源可追踪 |
...
1. 建 `Documentation~/`、`ShaderSystem/`、`Shaders/Generated/` 的 README 和表格空壳。
2. 建 `ShaderSystem/Contract/HORP_CONTRACT_INDEX.md`，链接 RP 侧契约。
3. 建 `ShaderSystem/FeatureBlocks/FEATURE_BLOCK_TABLE.md`，先登记最小原型 blocks。
4. 建 `ShaderSystem/Presets/PRESET_TABLE.md`，先登记 `Character_DebugLit_SSS_OITReady`。
