...
| `FeatureBlocks/` | 迁移期旧 `.honprblock` 声明目录；已迁入 `Features/` 的 block 不应保留副本。 |
...
`Features/**/*.honprblock`、`FeatureBlocks/**/*.honprblock`、`*.honprtemplate`、`*.honprpreset` 和 `Includes/INCLUDE_REGISTRY.honprinclude` 是人工维护的源声明。材质声明链路不再使用手写 JSON 作为胶水格式。
